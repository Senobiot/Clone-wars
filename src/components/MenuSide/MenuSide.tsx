 ...
 <div className={styles.heartbeatloader}>
      ...
      {['Главная', 'Медцентры', 'Врачи', 'Список услуг', 'Личный кабинет'].map((text, i) => (
             <Link key={i} to={
               ...
               i === 3 ? '/ServicesPage/' :
               '/PersonalPage/'
              } className={classes.link} color="white">
            <ListItem button key={text} color="white" className = {classes.container}>
              <ListItemIcon>{
              i  === 0 ?  <HomeIcon  className={classes.icon} /> :
              i === 1 ?   <BusinessIcon className={classes.icon} /> :
              i === 2 ?   <AssignmentIndIcon className={classes.icon}/> :
              i === 3 ?   <LocalHospitalIcon className={classes.icon}/> :
